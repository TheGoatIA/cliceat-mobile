...
class UserPrefsDao {
  ...
  Future<void> updateField(
    String userId,
    UserPrefsTableCompanion partial,
  ) async {
    await (_db.update(
      _db.userPrefsTable,
    )..where((t) => t.userId.equals(userId))).write(partial);
  }

  Future<void> deleteForUser(String userId) => (_db.delete(
    _db.userPrefsTable,
  )..where((t) => t.userId.equals(userId))).go();

  Future<void> setOnlineStatus(String userId, bool isOnline) => updateField(
    userId,
    UserPrefsTableCompanion(isOnline: drift.Value(isOnline)),
  );

  Future<void> setLanguage(String userId, String lang) =>
      updateField(userId, UserPrefsTableCompanion(language: drift.Value(lang)));
