...
class ChatRepository {
  ...
  Future<Either<AppError, List<ConversationModel>>> getConversations({
    ...
    try {
      ...
      if (res.isSuccessful && res.body != null) {
        ...
        // Save to cache
        await _chatDao.upsertConversations(list.map((c) => ConversationsTableCompanion.insert(
          id: c.id,
          type: c.type,
  ...
  Future<Either<AppError, List<MessageModel>>> getMessages(
    ...
    try {
      ...
      if (res.isSuccessful && res.body != null) {
        ...
        // Save to cache
        await _chatDao.upsertMessages(list.map((m) => MessagesTableCompanion.insert(
          id: m.id,
    ...
    try {
      final res = file != null
          ? await _chatService.sendMessageMultipart(
              conversationId, content, file.path)
          : await _chatService.sendMessage(conversationId, {'content': content});
      ...
      if (res.isSuccessful && res.body != null) {
        ...
        // Update cache
        await _chatDao.upsertMessages([MessagesTableCompanion.insert(
          id: model.id,
          conversationId: conversationId,
      ...
      if (file == null) { // We only queue text messages for now to avoid local file management complexity
        ...
        await _pendingActionsDao.addPending('send_chat', jsonEncode({
          'conversationId': conversationId,
          'content': content,
        }));
  ...
  List<ConversationModel> _fromConversationRows(List<ConversationsTableData> rows) {
    return rows.map((r) => ConversationModel(
      id: r.id,
      type: r.type,
      ...
      lastMessage: r.lastMessageContent != null ? MessageModel(
        id: '',
        conversationId: r.id,
        senderId: '',
        senderRole: '',
